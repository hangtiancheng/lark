...
description: >
  ...
  with createService, passing props to child views with *prop,
  ...
  ViewSetup, createStore, computed, bindStore, createService,
  createEmitter, createCache, createUpdater, createFrame, useState, useEffect,
  useStore, useUrlState, hotSwapByView, hotSwapByTemplate, hotSwapFrames,
...
| `ctx.beginUpdate(zoneId)`                       |                                                   | Begin zone update                                 |
| `ctx.endUpdate(zoneId, inner?)`                 |                                                   | End zone update                                   |
...
For `@event="handlerName"` bindings, `mountZone` registers `childFrame.on(eventName, parentHandler)` where `parentHandler` is found by prefix-matching the handler name in the parent's events map (e.g., `"increment"` matches `"increment<click>"`). The child fires events via `ctx.owner.fire("eventName", data?)`. Event matching is case-insensitive (the emitter lowercases event keys, so `fire("clearHistory")` matches `on("clearhistory")` from HTML-lowercased attribute names).

```ts
// Child view: fire event to parent
...
The service type `createService(syncFn, cacheMax?, cacheBuffer?)` creates a service with LFU caching, deduplication, and serial queueing. The sync function receives a `PayloadApi` object (internal, created by the framework) with `get(key)` and `set(keyOrData, value?)` methods.

## HMR (Hot Module Replacement)

Lark ships zero-config HMR for Vite, Webpack, and Rspack. The bundler plugins auto-inject HMR snippets — users never write `import.meta.hot` themselves. See `hmr.md` for full details.

### Auto-injected HMR

- For `.html` modules: self-accepts, calls `hotSwapByTemplate(old, new)` on update
- For `.ts` view modules: self-accepts, calls `hotSwapByView(old, new)` on update
- State is preserved across HMR swaps (same `ViewCtx`, same `updater.data`)
...
HMR swap functions are exposed via `globalThis.__lark_hmr__` so that auto-injected snippets can call them without importing `@lark.js/mvc` (avoids Module Federation chunk-graph side effects).
