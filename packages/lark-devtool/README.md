...
- CDN: interacts with lark-cdn to manage projects/versions, scan workspace dist directories, and publish in one click
- MF CDN: dynamically loads MF modules at runtime from any CDN remoteEntry.js

The service runs on port 5173. Designed to work alongside lark-demo (3000) and lark-cdn (3300).
...
- Node 18+
- pnpm 8+
- MongoDB 4+ (only required when using the CDN tab; CDN and MF CDN depend on lark-cdn)
...
# Terminal 2: lark-demo (required by MF Demo tab; MF CDN tab also needs lark-demo's dist)
cd lark-demo
pnpm dev:webpack
# -> http://localhost:3000/
...
Real-time inspection of any Lark application's Frame tree.
...
Manages projects and versions on lark-cdn.

```
+- CDN ----------------------------------------------------+
+------------------+---------------------------------------+
| Projects         | Discover & Publish [Scan Workspace]   |
|  v lark-demo  3v |  +- admin (dist-vite) v1.0.0 ------+ |
...
2 Click Scan Workspace to call `GET /api/discover`, which scans the lark-cdn workspace
...
To make your Lark application inspectable, two conditions must be met.
...
host load remote (lark-demo):
  1 fetch remoteEntry.js -> registers window.__lark_DemoMF = { init, get }
  2 await __webpack_init_sharing__("default")  // idempotent
  3 await lark_demo.init(__webpack_share_scopes__.default)
     -> remote inspects the share scope:
       - react / react-dom: uses the host's instance (singleton prevents duplication)
       - @lark.js/mvc: reuses the host's injected version or falls back to its own
  4 lark_demo.get("./counter-view")
     -> returns a factory
...
}>("http://localhost:3300/cdn/lark-demo/remoteEntry.js", "./counter-view");

const unmount = module.mountCounter(containerEl);

// Later: unmount + clear cache (so the next load issues a fresh request)
unmount();
clearRemoteCache("http://localhost:3300/cdn/lark-demo/remoteEntry.js");
```
...
1 extractContainerName(url)
   ...
   -> "http://...lark-demo/remoteEntry.js" -> "lark_demo"

2 inject <script src=URL>
   -> script.onload -> window.__lark_DemoMF = { init, get }
...
| -------------------------------------------- | ---------------------- | -------------- |
| `http://host/cdn/lark-demo/remoteEntry.js`   | `lark-demo`            | `lark_demo`    |
| `http://host/cdn/admin/remoteEntry.js`       | `admin`                | `admin`        |
| `http://host/cdn/admin@1.0.0/remoteEntry.js` | `admin@1.0.0`          | `admin@1_0_0`  |

Note the third row: if lark-cdn uses a `@version` suffix, the container name will include `@1_0_0`. This requires the lark-demo webpack config `name` to match exactly what the URL derives. Currently lark-demo uses `name: "lark_demo"`, which matches the `/cdn/lark-demo/remoteEntry.js` path. If `@version` URLs are used, the webpack config must be updated accordingly.
...
7 The default URL points to lark-demo's remoteEntry.js
...
lark-cdn uses an envelope response format:
...
Follow the instructions to start lark-demo.
...
The CDN URL is unreachable. Verify that lark-cdn is running, the URL path is correct, and CORS is enabled.

Error type 2: `Container "lark_demo" not found on window after loading <url>`

The remoteEntry.js loaded but did not register a global variable. Likely the lark-demo webpack `name` does not match the container name derived from the URL.

Error type 3: `mountCounter not found in remote module. Module keys: [...]`

The remoteEntry.js exposes a module without the mountCounter export. Check lark-demo's `exposes` path.

Error type 4: `Module "./counter-view" not found in container "lark_demo"`

The lark-demo expose path name does not match the URL. Check the `exposes` configuration in lark-demo's webpack config.

### 8.5 CDN Tab Shows "Failed to fetch"

lark-cdn is not running or is not on port 3300.
...
- `Invalid distPath`: the dist path is not within lark-cdn's configured `CDN_WORKSPACE_ROOT`
...
+-- src/
    ...
    +-- mf-types.d.ts                   // lark-demo/counter-view module declaration
...
- Lark: @lark.js/mvc (workspace internal dependency)
