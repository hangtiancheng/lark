...
export function setupGlobalSearch(options: UseGlobalSearchOptions) {
  ...
  const canShowResults = computed(
    () =>
      isOpen.value && trimmedQuery.value.length > 0 && status.value !== "idle",
  );
...
export const useGlobalSearch = createSetup(setupGlobalSearch);
